...
class EvmSyncSourceManager(
    ...
    private val evmNodeSyncManager: IEvmNodeSyncManager
) {
    ...
    fun defaultSyncSources(blockchainType: BlockchainType): List<EvmSyncSource> {
        return when (blockchainType) {
            BlockchainType.Ethereum -> {
                val syncNodes = evmNodeSyncManager.getNodes(1).map {
                    evmSyncSource(
                        blockchainType,
                        it.split("/").getOrNull(2)?.split(".").orEmpty().takeLast(2).joinToString("."),
                        RpcSource.Http(listOf(URI(it)), null),
                ...
                listOf(
                    evmSyncSource(
                        blockchainType,
                        "Ankr",
                        RpcSource.Http(listOf(URI("https://rpc.ankr.com/eth")), null),
                        defaultTransactionSource(blockchainType)
                    ),
                    evmSyncSource(
                        blockchainType,
